...
void setup() {
  ...
  *portb = 0xFF;
  ...
  // Letter bitmaps (column-wise)
  char W[8] = {0x81, 0xA5, 0xA5, 0xBD, 0xBD, 0xBD, 0xFF, 0xFF}; // W
char E[8] = {0xC7, 0xBF, 0x83, 0xBB, 0xC7, 0xFF, 0xFF, 0xFF}; // E
char L[8] = {0x83, 0xEF, 0xEF, 0xEF, 0xEF, 0xEF, 0xCF, 0xFF}; // L
char C[8] = {0xC7, 0xBB, 0xBF, 0xBF, 0xBB, 0xC7, 0xFF, 0xFF}; // C
char O[8] = {0xCF, 0xB7, 0xB7, 0xCF, 0xFF, 0xFF, 0xFF, 0xFF}; // O
char M[8] = {0xAB, 0xAB, 0xAB, 0xAB, 0xAB, 0xD7, 0xFF, 0xFF}; // M
    ...
    for (int shift= 0; shift<totalrows; shift++) {
      for (int frame = 0; frame < 50; frame++) {
        for (int row = 0; row < 8; row++) {
          ...
          *porta = rows[(shift + row) % totalrows];  // Scroll across
          delay1();
          *porta = 0x00;
          *portb = 0xFF;
        }
